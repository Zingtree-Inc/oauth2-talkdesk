<?php
namespace Zingtree\OAuth2\Client\Provider;
...
class Talkdesk extends AbstractProvider
{
    use BearerAuthorizationTrait;

    protected $subdomain;

    private $domain = '.talkdeskid.com';
    private $apiUrl = 'https://api.talkdeskapp.com';
    ...
    public function __construct(array $options = [], array $collaborators = [])
    {
        ...
        if (empty($this->subdomain)) {
            throw new Exception\ProviderConfigurationException(
                'No subdomain has been configured for this Talkdesk provider; it has to have a subdomain.'
            );
        }
    }
     ...
     * @param AccessToken $token
     * @return TalkdeskResourceOwner
     */
    protected function createResourceOwner(array $response, AccessToken $token)
    {
        return new TalkdeskResourceOwner($response);
    }
